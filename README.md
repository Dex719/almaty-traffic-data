...
Оборванная загрузка оставляет в релизе актив в состоянии `starter`: API его
перечисляет, а `gh release download` и `gh release delete-asset` не видят.
...
завершается кодом 1. Сохраните такой каталог (например, артефакт восстановления).

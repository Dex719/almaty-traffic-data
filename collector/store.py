...
Вахтовый режим (``monthly=True``) — актуальные пути:

* ``data/scores/YYYY-MM.csv`` — по строке на завершившуюся группу опросов:
  баллы обоих сервисов, тренд Яндекса, длина затруднений, счётчики событий.
* ``data/events/YYYY-MM.json`` — месячный реестр событий: ``layer:id`` →
  карточка с ``first_seen`` / ``last_seen``; событие, пережившее границу
  месяца, переезжает в новый файл с сохранением ``first_seen``.
* ``data/snapshots/YYYY-MM/DD.jsonl`` — снимок «какие события активны сейчас»
  на каждый замер по алматинской дате (в Actions публикуется через Releases).

Legacy ``data/scores.csv`` и ``data/events.json`` (``monthly=False``) заморожены:
не пополняются и не удаляются; ``events.json`` читается только как источник
``first_seen`` при миграции.
"""
